...
- **Secure Identity HAL**: Pluggable trait-based `IdentityVault` for cryptographic operations, paving the way for TPM and cloud-hosted hardware signing. Agent identity is currently exposed as `did:aria:jayesh` — a stub alias; the production method resolves to a real `did:hedera` DID anchored on-chain via [`hiero-did-sdk-rs`](https://github.com/hiero-hackers/hiero-did-sdk-rs), a Rust DID SDK published to crates.io.
